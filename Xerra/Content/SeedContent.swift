...
enum SeedContent {
    ...
    /// the plain verb. The hard cases are in on purpose — years in England and
    /// a whole night are both still dots.
    ...
    /// it. The hard cases are in on purpose — 'viví muchos años', 'no pude
    ...
    /// Vocabulary by the keyword method, ported from Deb-o-lingo's Palabras
    /// unit and written fresh for Catalan.
    ///
    /// Every other deck here teaches something you say. These teach single
    /// words, the way people who are good at this actually learn them: you
    /// hear an English sound inside the Catalan word, and you build one absurd
    /// picture out of that sound and the meaning. `la clau` sounds like a
    /// clown with the n bitten off, so a clown holds your key in his teeth,
    /// and the word stops being a coin-flip.
    ///
    /// Two fields carry it. `sounds` is the bridge — what the word sounds like
    /// in English, and nothing else. `picture` is the scene, and it has one
    /// job: to hold BOTH the sound and the meaning, so that remembering the
    /// scene hands the word back. A scene with the sound but no meaning is
    /// useless; so is a pretty one with neither.
    ///
    /// Rules for writing more:
    ///   - Strange beats sensible. Impossible, violent or rude is better than
    ///     plausible; a sensible picture is forgotten by Thursday.
    ///   - Never bridge to a sound the word hasn't got. `la clau` is not
    ///     "claw", however good the picture would be — au is the vowel in
    ///     'cow', and a mnemonic that teaches the wrong mouth is worse than no
    ///     mnemonic at all in an app that scores pronunciation for a living.
    ///     `l'escala` is not "escalator" and `la maleta` is not "mallet" for
    ///     the same reason one level down: English puts the stress on the
    ///     front of both, Catalan puts it in the middle, and in Catalan stress
    ///     is what decides which vowels survive.
    ///   - The `focusNote` still does the real pronunciation work. The picture
    ///     only has to get you to the word.
    ///   - One picture per word, one sentence long. It is a hook, not a story.
    ///
    /// The nouns carry their article — "la forquilla", not "forquilla" —
    /// because a noun learnt without its gender has to be learnt twice.
    static var catalanWords: [Phrase] {
        wordsTable + wordsStreet + wordsEveryDay
            + wordsQuestions + wordsClock + wordsAway
    }

    static let wordsTable: [Phrase] = [
        Phrase(
            text: "la forquilla",
    ...
    static let wordsEveryDay: [Phrase] = [
        Phrase(
            text: "tenir",
            translation: "to have",
    ...
    static let wordsQuestions: [Phrase] = [
        Phrase(
            text: "on?",
            translation: "where?",
